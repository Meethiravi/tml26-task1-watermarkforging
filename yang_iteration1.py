...
for source_wm, target_start, target_stop in CATEGORIES:
    ...
    # ----------------------------------------------------------------
    # YANG ET AL. IMPROVEMENT:
    # Instead of subtracting a scalar DC offset (Kutter),
    # subtract the mean of actual clean images resized to match source.
    #
    #   watermark = mean(watermarked) - mean(clean)
    #
    # The clean mean is a much better estimate of the "average image
    # content" that averaging doesn't fully cancel with only 25 samples.
    # Using all 200 clean targets gives the best content estimate.
    # ----------------------------------------------------------------
    clean_resized = []
    for c in clean_pool:
        c_h, c_w = c.shape[:2]

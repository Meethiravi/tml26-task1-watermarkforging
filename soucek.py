"""
Adapted from: Souček et al. (2025) "Transferable Black-Box One-Shot Forging of
Watermarks via Image Preference Models", NeurIPS 2025.

Key adaptations for our constraints (25 images, single P100):
- ConvNeXt V2-Tiny → lightweight ResNet18 (pretrained, fine-tuned head only)
- SA-1B dataset (millions) → 200 clean target images
- 120k steps on 8xV100 → ~1000 steps on 1xP100
- Full backprop through model → same (this part is identical to the paper)

What's faithful to the paper:
- Preference model trained on synthetic artifacts (no real watermarks needed)
- Ranking loss: clean image scores lower than artifact image
- Watermark extraction via backprop through the preference model
- Forging by adding extracted watermark to clean target
"""
...
# Hyperparameters
PREF_MODEL_STEPS = 1000     # preference model training steps
PREF_LR = 1e-4              # preference model learning rate
EXTRACT_STEPS = 100         # steps to extract watermark from source images
EXTRACT_LR = 0.05           # lr for watermark extraction
FORGE_STEPS = 100           # steps to forge watermark onto target
FORGE_LR = 0.05             # lr for forging
LAMBDA_QUALITY = 8.0        # quality preservation weight during forging
...
# ----------------------------------------------------------------
# PREFERENCE MODEL
# Souček: ConvNeXt V2-Tiny trained from scratch on SA-1B dataset.
# Our adaptation: pretrained ResNet18 with fine-tuned head,
# trained on our 200 clean images + synthetic artifacts.
# The preference model R(x) outputs a single score:
#   high score = image looks watermarked (has hidden artifacts)
#   low score  = image looks clean
# ----------------------------------------------------------------
...
# Souček Section 3.1: generate synthetic artifacts procedurally.
# Key insight: train on fake artifacts, not real watermarks.
# Types used in the paper: sinusoidal patterns, random noise,
# grid patterns — all similar to what real watermarks look like.
# ----------------------------------------------------------------
def generate_synthetic_artifact(batch_size, h=IMG_SIZE, w=IMG_SIZE):
    ...
    for _ in range(batch_size):
        ...
        if artifact_type == 0:
            # Sinusoidal pattern (Fourier-domain watermarks)
            ...
            # Random noise pattern (spread-spectrum watermarks)
            pattern = np.random.randn(h, w, 3).astype(np.float32)
            pattern /= (np.abs(pattern).max() + 1e-8)

        else:
            # Grid pattern (block-based watermarks)
        ...
        # Scale artifact strength (watermarks are typically small)
...
# Souček Eq. 1 — Ranking loss:
#   L = max(0, R(x_clean) - R(x_artifact) + margin)
# Model learns: artifact images should score HIGHER than clean ones.
# We train entirely on synthetic artifacts + clean images.
# No real watermarks are needed at this stage.
# ----------------------------------------------------------------
def train_preference_model(clean_tensors, steps=1000, lr=1e-4):
    ...
    clean_stack = torch.cat(clean_tensors, dim=0)  # (N, 3, H, W)

    print(f"  Training preference model for {steps} steps...")
    model.train()

    for step in range(steps):
        # Sample random clean images
        idx = torch.randint(0, n_clean, (8,))
        x_clean = clean_stack[idx]  # (8, 3, H, W)

        # Generate synthetic artifacts and add to clean images
        artifact = generate_synthetic_artifact(8, h=IMG_SIZE, w=IMG_SIZE)
        x_artifact = (x_clean + artifact).clamp(-1, 1)

        # Ranking loss: R(artifact) should be > R(clean) + margin
...
# ----------------------------------------------------------------
# EXTRACT WATERMARK FROM SOURCE IMAGES
# Souček: optimize delta to REMOVE the watermark signal.
#   delta* = argmin R(x_wm - delta) + lambda * ||delta||
# The optimal delta IS the watermark estimate.
# We run this on all 25 source images and average.
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# FORGE WATERMARK ONTO TARGET
# Souček: optimize target image to score HIGH on preference model
# while staying close to the original clean target.
#   forged* = argmax R(target + delta)
#             s.t. ||delta||_perceptual < epsilon
# ----------------------------------------------------------------
...
# ----------------------------------------------------------------
# STAGE 1: Train ONE preference model on clean images + synthetic artifacts
# This is shared across all 8 WM methods — no real watermarks needed.
# ----------------------------------------------------------------
print("\n" + "="*60)
...
for source_wm, target_start, target_stop in CATEGORIES:
    ...
    # ----------------------------------------------------------------
    # STAGE 2: Extract watermark from source images via backprop
    # ----------------------------------------------------------------
    ...
    # ----------------------------------------------------------------
    # STAGE 3: Forge watermark onto each target image
    # ----------------------------------------------------------------
...
print("\nTuning guide:")
print("  PREF_MODEL_STEPS: more = better preference model")
print("  EXTRACT_STEPS:    more = cleaner watermark extraction")
print("  FORGE_STEPS:      more = stronger forgery")

...
TEMP_OUT_DIR = BASE_DIR / "submission_temp_soucek_iter2"
FILE_PATH = str(BASE_DIR / "submission_soucek_iter2.zip")
...
def load_tensor_at(path, h, w):
    """Load an image resized to an arbitrary (h, w) -- used to bring SOURCE
    images to the batch's target resolution BEFORE extraction, rather than
    forcing everything through a fixed 256x256 and resizing the result
    afterward. PreferenceModel accepts any resolution (ResNet's final layer
    is AdaptiveAvgPool2d), so there's no need for a fixed working size here."""
...
def extract_watermark(source_tensors, preference_model, steps=150, lr=0.01, eps=0.06):
    ...
    return watermark  # (1, 3, target_h, target_w) -- native resolution of this batch


# ----------------------------------------------------------------------
# FORGE WATERMARK — fully native resolution, hard epsilon projection
# ----------------------------------------------------------------------
def forge_watermark(target_native, watermark_pattern_native, preference_model,
                     steps=150, lr=0.01, lambda_quality=50.0,
                     eps=0.06, max_quality_loss=0.01):
    """
    target_native: (1,3,H,W) full-resolution tensor in [-1,1].
    watermark_pattern_native: (1,3,H,W) starting delta, ALREADY at this
    target's resolution (see main loop -- extraction is now done per-batch
    at the batch's native resolution, so no resize is needed here at all).

    Because PreferenceModel's backbone ends in AdaptiveAvgPool2d, it can
    score images of any resolution directly -- no downsampled proxy view
    needed. This removes a full resize round-trip that the earlier version
    required (optimize at 256 -> upsample delta -> apply -> downsample a
    view for scoring). Fewer resizes = less resize-induced LPIPS cost and
    a cleaner match between what's optimized and what's scored.
    """
    preference_model.eval()
    target_native = target_native.to(DEVICE)

    delta = nn.Parameter(watermark_pattern_native.to(DEVICE).clone())
    optimizer = optim.Adam([delta], lr=lr)

    for step in range(steps):
        optimizer.zero_grad()

        forged_native = (target_native + delta).clamp(-1, 1)

        pref_loss = -preference_model(forged_native).mean()
    ...
    with torch.no_grad():
        forged_final = (target_native + delta).clamp(-1, 1)
...
for source_wm, target_start, target_stop in CATEGORIES:
    ...
    # Detect this batch's native TARGET resolution from its first clean
    # image (assumes uniform resolution within a batch, which held true
    # for the confirmed WM_1-8 -> resolution mapping: 256x256 for most,
    # 128x128 for WM_5, 512x512 for WM_7/WM_8). Source images are then
    # resized to THIS resolution before extraction, rather than extracting
    # at a fixed 256 and resizing the delta afterward -- avoids warping the
    # watermark's spatial/frequency structure via a post-hoc interpolation,
    # especially significant for the 256->512 upsample batches.
    first_target = Image.open(clean_dir / f"{target_start}.png")
    target_w, target_h = first_target.size
    print(f"  batch native resolution: {target_w}x{target_h}")

    src_native_size = Image.open(source_paths[0]).size
    if src_native_size != (target_w, target_h):
        print(f"  [note] resizing sources from {src_native_size} to "
              f"{(target_w, target_h)} before extraction (mismatched resolution)")

    source_tensors_native = [load_tensor_at(p, target_h, target_w) for p in source_paths]

    print(f"  Stage 2: Extracting watermark from {len(source_tensors_native)} source images "
          f"at {target_w}x{target_h} (eps={EXTRACT_EPS})...")
    watermark_pattern = extract_watermark(
        source_tensors_native, preference_model,
